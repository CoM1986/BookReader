...
namespace FB2Library.Elements
{
    ...
    public class BinaryItem
    {
        ...
        internal async void Load(XElement binarye)
        {
            if (binarye == null)
            {
            ...
            content = await DetectContentType(BinaryData);
        ...
        private async Task<ContentTypeEnum> DetectContentType(byte[] binaryData)
        {
            ...
            try
            {
                InMemoryRandomAccessStream imgStream = new InMemoryRandomAccessStream();
                await imgStream.WriteAsync(binaryData.AsBuffer());
                var bitmapDecoder = await BitmapDecoder.CreateAsync(imgStream).AsTask().ConfigureAwait(false);
                var codecid = bitmapDecoder.DecoderInformation.CodecId;

                if (codecid.Equals(BitmapDecoder.JpegDecoderId))
                {
                    contentType = ContentTypeEnum.ContentTypeJpeg;
                }
                else if (codecid.Equals(BitmapDecoder.PngDecoderId))
                {
                    contentType = ContentTypeEnum.ContentTypePng;
                }
                else if (codecid.Equals(BitmapDecoder.GifDecoderId))
                {
                    contentType = ContentTypeEnum.ContentTypeGif;
                }
